...
#pragma warning restore 0414
    ...
    public void OnInitialized(IStoreController controller, IExtensionProvider extensions)
    {
		...
		client.Headers.Add ("Content-Type", "text/json; charset=utf-8");
		client.Headers.Add("User-Agent", 
			               "Unity version: " + Application.unityVersion + " " + 
			               "IAP version: "   + StandardPurchasingModule.k_PackageVersion);

		string result = "{'Platform':'" + Application.platform + "',";
		...
		/*-----------------------------------------------------------------------------------------*/
				...
				"','description':'" + item.metadata.localizedDescription +
				"','isoCurrencyCode':'" + item.metadata.isoCurrencyCode +
				"','price':'" + item.metadata.localizedPrice.ToString () +
				"','localizedPrice':'" + item.metadata.localizedPriceString +
				"','transactionID':'" + item.transactionID +

				count++;
				
				if (count < controller.products.all.Length) 
				{
					result += "'},";
				} 
				else 
